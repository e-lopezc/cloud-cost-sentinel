...
Below is a sample of the HTML report generated by Cloud Cost Sentinel. This particular run was executed against an AWS account with no active resources, so no findings are shown — in a real environment, the report would list idle EC2 instances, unattached EBS volumes, forgotten RDS snapshots, and more, each with an estimated monthly cost.

![Cloud Cost Sentinel — Sample HTML Report](cloud_sentinel_sample_report.png)
